...
public:
   ...
   bool retrieve(int index, objType& dataItem) const;
   //doesn’t allow for changes, returns false if bad index or NULL
   
   objType* access(int index); //returns address of theObj at ListNode index 
 ...
 */
template <typename objType>
bool List<objType>::retrieve(int index, objType& theObj) const
{
    if ((index < 1) || (index > getLength()))
    {
        return false;
...
/**
 * Return the address of theObj in the listNode at index and allow direct
 * editing by user... kind of dangerous but hugely helpful
 * @pre 1 <= index <= getLength()
 * @post If 1 <= index <= getLength(), returns address
 * @param index the list position to retrieve address of theObj
 * @return address of an objType if valid index NULL if bad index
 */
template <typename objType>
objType* List<objType>::access(int index)
{
    ListNode *current = head;
    objType tempObj;
    if ((index < 1) || (index > getLength()))
    {
        std::cout <<"You used a bad index and are now editing the head node"
                  << std::endl;
        return &head->theObj;
    } else
    {//get pointer to node, then data in node
